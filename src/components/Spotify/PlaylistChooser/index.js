                    ...
                    <Col>
                        ...
                        <ul style={{margin:'0',padding:'0'}}> 
                            {playlists && playlists.map(playlist => {
                                return (<li key={playlist.id} style={{listStyleType:'none', marginBottom:'1rem'}}>{playlist.name}<span style={{display:'inline-block','float':'right','clear':'both'}}><Button size="sm" style={{marginRight:'0.5rem'}} onClick={() => this.selectPlaylist(playlist)}>Select</Button><Button size="sm">Tracks</Button></span></li>)
                            })}                    
                        </ul>
                    </Col>
                </Row>
